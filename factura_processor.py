...
"""
AMEXTRON Factura Processor GUI

This script provides a graphical user interface to process AMEX invoice packages.
It allows the user#  to:
1. Specify a 'personaje' (owner) and 'mes' (month).
2. Select a ZIP file containing XML and PDF invoices.
3. Process the files: extract data from XMLs, rename files based on UUID,
...
class App:
    ...
    def __init__(self, root_window):
        ...
        self.root.title("Procesador de Facturas AMEXTRON")
        ...
        # --- Input Fields ---
        fields_frame = tk.LabelFrame(main_frame, text="Datos de Entrada", padx=10, pady=10, bg="#f0f0f0", font=("Helvetica", 12))
        fields_frame.pack(fill=tk.X, pady=(0, 10))

        tk.Label(fields_frame, text="Personaje (Dueño):", bg="#f0f0f0", font=("Helvetica", 10)).grid(row=0, column=0, sticky="w", pady=5, padx=5)
        ...
        tk.Label(fields_frame, text="Mes de Análisis:", bg="#f0f0f0", font=("Helvetica", 10)).grid(row=1, column=0, sticky="w", pady=5, padx=5)
    ...
    def start_processing_thread(self):
        ...
        if not personaje or not mes:
            messagebox.showerror("Error de Entrada", "Los campos 'Personaje' y 'Mes' no pueden estar vacíos.")
            return
    ...
    def process_files(self, personaje, mes, zip_path):
        """
        The core logic for processing the files. This function is run in a
        separate thread.
        """
        temp_dir = None
        try:
            ...
            output_zip_name = f"PROCESADO_{personaje}_{mes}.zip"
